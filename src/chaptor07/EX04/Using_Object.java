package chaptor07.EX04;

class Person {
	// class : 객체를 생성하기 위한 템플릿(틀) , 설계도, 붕어빵기계
	// Object(객체,인스턴스) : 건축물, 붕어빵 <== 인스턴스화 (
	
	// 1. 필드 : 클래스 블락에서 선언된 변수(Heap) . 지역변수 : 메소드 블락에서 선언된 변수 (stack)
	
	String name ;  // 이름 			: 기본값 null
	float height;   // 키 (실수) 		: 기본값 0.0
	float weight;  // 몸무게 (실수)	: 기본값 0.0
	int age;       // 나이 (정수)		: 기본값 0 
...
public class Using_Object {
	...
	public static void main(String[] args) {
		...
		// show() : 필드의 내용을 출력
		System.out.println("show () 메소드를 통해 출력");
		person.show();
		...
		Lee.show();
		// 3. 매개변수 4개인 생성자 호출
		System.out.println("==== 매개변수 4개 출력===");
		Person hong = new Person("홍길동" , 185.5f , 39.3f , 43);
		...
		//setter 를 사용해서 필드값 할당
		jung.setName("정약용");
		jung.setHeight(188.3f);
		jung.setWeight(75.3f);
		jung.setAge(42);
