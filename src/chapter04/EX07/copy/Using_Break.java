...
public class Using_Break {
	...
	public static void main(String[] args) {
		...
		//break는 for,while, do while 내에서 if 조건과 같이 사용
		for (int i = 0; i<10; i++) {
			System.out.println(i);
		...
		for ( int i = 0; i <5; i++) {  // 0~4 : 5번 loop 
		 ...
		 // case 2. 한꺼번에 탈출 ( 바깥쪽 for 문의 변수의 값을 조건이 false가 나도록 증가 시켜서 빠져나감 )
		System.out.println("=====================");
		for ( int i = 0; i <5; i++) {  // 0~4 : 5번 loop 
			for ( int j = 0; j<5; j++) { // 0~4 : 5번 loop 
				if (j==2) {
		...
		System.out.println("========이중 for문 탈출(label)==========");
