...
public class Using_Method_Overriding02 {
   ...
   public static void main(String[] args) {
      ...
      // 2. Animal 타입으로 선언 + 자식 클래스로 객체 생성
         // 자식의 객체를 생성할 때 부모 타입으로 정의 후 부모의 메소드를 호출 시 오버라이딩 된 메소드 출력
      ...
      bb1.cry();   //bb1.cry() : Animal 의 cry()를 호출   --> 오버라이딩 된 메소드가 출력
      cc1.cry();   //cc1.cry() : Animal 의 cry()를 호출   --> 오버라이딩 된 메소드가 출력
      dd1.cry();   //dd1.cry() : Animal 의 cry()를 호출   --> 오버라이딩 된 메소드가 출력
      
      //Animal 타입으로 선언이 되어 있으므로 배열이나 컬렉션에 넣어서 아주 쉽게 관리
      //배열의 특징 : 1. 동일한 타입을 저장, 2. 방의 크기가 선언되면 바꿀 수 없다.
      
      int [] arr1 = new int[5];         //배열의 방크기 지정 후 값을 할당
      ...
      for (int i =0; i < arr3.length ; i++) {
         arr3[i].cry();      //Animal에 있는 cry() 를 호출시 객체에서 오버라이딩 된 메소드가 출력
      }
